...
export function dynamicRegisterEnum<T extends object>(
  ...
  try {
    ...
    const valuesMap: Record<string, { description?: string; deprecationReason?: string }> = {};
    ...
    stringKeys.forEach((key) => {
      ...
      // 2. Auto-generate description
        ...
        valueDescription = descriptionPrefix ? `${descriptionPrefix}${formatted}` : formatted;
      }
...
export function batchRegisterEnums<T extends object>(
  ...
  logger.log(`✅ Batch registration complete: ${successCount} success, ${failureCount} failed`);
}
